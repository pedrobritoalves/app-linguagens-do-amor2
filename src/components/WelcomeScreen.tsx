...
import { type Gender, type Mode, genderEmoji, themes } from '../theme';
  ...
  return (
    <div className="animate-fade-in-up mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center px-6 py-12 text-center">
      ...
      <p className="mb-8 max-w-xl text-base leading-relaxed text-sand-700 sm:text-lg">
        Todos expressamos e recebemos amor de formas diferentes. Em apenas 15 perguntas rápidas,
        você vai descobrir qual é a sua linguagem predominante — e como usá-la para transformar
          ...
          {modes.map((m) => {
            ...
            return (
              ...
              >
                ...
                {m.mode === 'couple' && (
                  <span className="flex gap-0.5 text-lg" aria-hidden>
                    <span>{genderEmoji.female}</span>
                    <span>{genderEmoji.male}</span>
                  </span>
                )}
              </button>
            );
      ...
      <p className="mt-5 text-xs text-sand-500">Leva cerca de 3 minutos · 15 perguntas</p>
